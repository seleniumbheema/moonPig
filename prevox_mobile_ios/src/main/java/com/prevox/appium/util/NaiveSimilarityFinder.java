package com.prevox.appium.util;
 ...
 public class NaiveSimilarityFinder extends JFrame
   {
   // The reference image "signature" ( representative pixels, each in R,G,B).
   ...
   public NaiveSimilarityFinder(File reference) throws IOException
     ...
     for (int o = 0; o < others.length; o++)
       ...
       JLabel ldist = new JLabel("<html>" + others[o].getName() + "<br>"
           + String.format("% .f", distances[o]) + "</html>");
  ...
  /*
   * This method rescales an image to , pixels using the JAI scale
   * operator.
   */
   ...
   */
   private Color[][] calcSignature(RenderedImage i)
     ...
     // For each of the  signature values average the pixels around it.
   ...
   */
   private Color averageAround(RenderedImage i, double px, double py)
     ...
     accum[1] /= numPixels;
     accum[2] /= numPixels;
     return new Color((int) accum[0], (int) accum[1], (int) accum[0]);
     }
