@SmokeCreateNewConversationSearch
Feature: Composing in conversation, Using text Tool using single element
  ...
  Scenario Outline: Select UserName from searchButton
    When I click on "searchButton" with name 
    And I enter "typeString" as "threeCharacters"
    And I Select "userSearch" from "selectedUserString" on serachResults
